...
import websocket
from .sessiongate import SessionGate
from .shapes import ChatMessage, Chat, ItemDeal
from .kinds import IngressPoint, RoomKind
from .graphfrag import chat, chat_message, QUERIES
...
class WireEnvelope:
    ...
    def __init__(self, event_type: IngressPoint, chat: types.Chat):
        self.type = event_type
        self.chat = chat
        self.time = time_module.time()


class WireRoomSnapshot(WireEnvelope):

    def __init__(self, chat: types.Chat):
        super().__init__(IngressPoint.ROOM_SYNCED, chat)
        self.chat: types.Chat = chat


class WireChatLine(WireEnvelope):

    def __init__(self, message: types.ChatMessage, chat: types.Chat):
        super().__init__(IngressPoint.CORR_INBOUND, chat)
        self.message: types.ChatMessage = message


class WireDealOpened(WireEnvelope):

    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super().__init__(IngressPoint.LEDGER_OPEN, chat)
        self.deal: types.ItemDeal = deal


class WireRatingAdded(WireEnvelope):

    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super().__init__(IngressPoint.STARS_NEW, chat)
        self.deal: types.ItemDeal = deal


class WireRatingPulled(WireEnvelope):

    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super().__init__(IngressPoint.STARS_REMOVED, chat)
        self.deal: types.ItemDeal = deal


class WireRatingEdited(WireEnvelope):

    def __init__(self, deal: types.ItemDeal, chat: types.Chat, previous_fp: str):
        super().__init__(IngressPoint.STARS_AMENDED, chat)
        self.deal: types.ItemDeal = deal
        self.previous_fp: str = previous_fp


class WireDealClosed(WireEnvelope):

    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super().__init__(IngressPoint.LEDGER_SETTLED, chat)
        self.deal: types.ItemDeal = deal


class WireDealUndone(WireEnvelope):

    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super().__init__(IngressPoint.LEDGER_REVERSED, chat)
        self.deal: types.ItemDeal = deal


class WireDealDispute(WireEnvelope):

    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super().__init__(IngressPoint.LEDGER_DISPUTE, chat)
        self.deal: types.ItemDeal = deal


class WireDisputeResolved(WireEnvelope):

    def __init__(self, deal: types.ItemDeal, chat: types.Chat, resolver_username: str | None = None):
        super().__init__(IngressPoint.LEDGER_DISPUTE_CLEARED, chat)
        self.deal: types.ItemDeal = deal
        self.resolver_username: str | None = resolver_username


class WireDealPhase(WireEnvelope):

    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super().__init__(IngressPoint.LEDGER_STAGE, chat)
        self.deal: types.ItemDeal = deal


class WireSkuFunded(WireEnvelope):

    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super().__init__(IngressPoint.SKU_PAID, chat)
        self.deal: types.ItemDeal = deal


class WireSkuDispatched(WireEnvelope):

    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super().__init__(IngressPoint.SKU_SHIPPED, chat)
        self.deal: types.ItemDeal = deal


class TapStream:

    def __init__(self, conn: SessionGate):
        self.conn: SessionGate = conn
        ...
        if message.text == '{{ITEM_PAID}}':
            ...
            if actual_msg and actual_msg.deal:
                ...
                if deal_id not in self.processed_deals:
                    ...
                    return []
                return [WireDealOpened(actual_msg.deal, chat_obj), WireSkuFunded(actual_msg.deal, chat_obj)]
        elif message.text == '{{ITEM_SENT}}':
            actual_msg = self._get_actual_message(message.id, chat_obj.id) or message
            if actual_msg and actual_msg.deal:
                return [WireSkuDispatched(actual_msg.deal, chat_obj), WireDealPhase(actual_msg.deal, chat_obj)]
            ...
            if actual_msg and actual_msg.deal:
                ...
                if deal_id not in self.review_check_deals and deal_id not in self.review_watch_deals:
                    self.review_check_deals.append(deal_id)
                return [WireDealClosed(actual_msg.deal, chat_obj), WireDealPhase(actual_msg.deal, chat_obj)]
        elif message.text == '{{DEAL_ROLLED_BACK}}':
            actual_msg = self._get_actual_message(message.id, chat_obj.id) or message
            if actual_msg and actual_msg.deal:
                return [WireDealUndone(actual_msg.deal, chat_obj), WireDealPhase(actual_msg.deal, chat_obj)]
        elif message.text == '{{DEAL_HAS_PROBLEM}}':
            actual_msg = self._get_actual_message(message.id, chat_obj.id) or message
            if actual_msg and actual_msg.deal:
                return [WireDealDispute(actual_msg.deal, chat_obj), WireDealPhase(actual_msg.deal, chat_obj)]
        elif message.text == '{{DEAL_PROBLEM_RESOLVED}}':
            actual_msg = self._get_actual_message(message.id, chat_obj.id) or message
            if actual_msg and actual_msg.deal:
                ru = getattr(getattr(actual_msg, 'user', None), 'username', None)
                return [WireDisputeResolved(actual_msg.deal, chat_obj, resolver_username=ru)]
        return [WireChatLine(message, chat_obj)]
    ...
    def _apply_deal_event_cooldown(self, events: list) -> list:
        ...
        for ev in events:
            ...
            if t in (IngressPoint.LEDGER_DISPUTE_CLEARED, IngressPoint.LEDGER_DISPUTE):
    ...
    def _proccess_new_chat_message(self, chat_obj, message):
        ...
        if not is_subscribed:
            ...
            if is_new_chat:
                events.append(WireRoomSnapshot(chat_obj))
            ...
            for deal_id in list(self.review_check_deals):
                try:
                    ...
                    if fp and old is None:
                        ...
                        yield WireRatingAdded(deal, deal.chat)
                except Exception:
            ...
            for deal_id in list(self.review_watch_deals):
                try:
                    ...
                    if old and not fp:
                        ...
                        self._resolve_deal_chat(deal)
                        yield WireRatingPulled(deal, deal.chat)
                    elif fp and old and fp != old:
                        self.review_snapshots[deal_id] = fp
                        self._resolve_deal_chat(deal)
                        yield WireRatingEdited(deal, deal.chat, previous_fp=old)
