...
class MarketBridge:
    ...
    def _reactivate(self, item: Item | MyItem | ItemProfile) -> None:
        try:
            ...
            try:
                tier = next((s for s in tiers if s.type == BoostLevel.DEFAULT or s.price == 0), tiers[0])
            ...
            if new_item.status in (ListingStage.PENDING_APPROVAL, ListingStage.APPROVED):
                ...
                logger.error('Не удалось восстановить «%s» — статус: %s', short, new_item.status.name)
        except Exception as e:
    ...
    async def _on_paid(self, event: ListingPaidNotice) -> None:
        ...
        if event.deal.user.id == self.account.id:
            return
        if not self.config['auto']['restore']['sold']:
            return
        ...
        if item is None or getattr(item, 'status', None) != ListingStage.SOLD:
            logger.debug('[reactivate/sold] лот %s ещё не SOLD, пропуск', item_id)
            return
        self._reactivate(item)
