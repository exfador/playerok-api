...
class MarketBridge:
    ...
    def _reactivate(self, item: Item | MyItem | ItemProfile, retry_delays: list[int] | None = None) -> None:
        try:
            ...
            tiers = self.account.load_boost_tiers(item.id, item.price)
            allow_premium = self.config.get('auto', {}).get('restore', {}).get('premium', False)
            try:
                ...
                if free_tier:
                    ...
                    logger.warning('Лот «%s» требует платного тира (%s₽), но опция PREMIUM отключена', item.name[:32], tiers[0].price if tiers else '?')
                    return
            except (StopIteration, IndexError):
                logger.warning('Не удалось получить тарифы для «%s»', item.name[:32])
                return
            short = item.name[:32] + ('...' if len(item.name) > 32 else '')
