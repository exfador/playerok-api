...
from pok.feed import Feed, ChatInitializedEvent, NewMessageEvent, NewReviewEvent, ReviewRemovedEvent, ReviewUpdatedEvent, ItemPaidEvent, NewDealEvent, DealStatusChangedEvent, DealHasProblemEvent, DealProblemResolvedEvent
from lib.consts import ACCENT_COLOR, VERSION, C_PRIMARY, C_SUCCESS, C_WARNING, C_ERROR, C_DIM, C_TEXT, C_BRIGHT, C_HIGHLIGHT
from lib.util import set_console_title, halt, spawn_async, draw_box, _b_top as _box_top, _b_bot as _box_bot, iso_to_display_str
from lib.bus import on_sys, on_mkt, dispatch, dispatch_market
...
logger = getLogger('pl.bot')


def active_engine() -> Runtime | None:
    if hasattr(Runtime, 'instance'):
        return getattr(Runtime, 'instance')


class Runtime:

    def __new__(cls, *args, **kwargs) -> Runtime:
    ...
    def _run_custom_chat_command(self, raw_text: str, chat_id: str, username: str) -> None:
        ...
        for ev in item.get('events') or []:
            if ev == 'call_seller':
                asyncio.run_coroutine_threadsafe(_get_panel().call_seller(username, chat_id), _get_panel_loop())
                self.send_message(chat_id, self.msg('cmd_seller'))
        rl = [x for x in (item.get('reply_lines') or []) if str(x).strip()]
        if rl:
            self.send_message(chat_id, '\n'.join(rl))

    async def _on_chat_initialized(self, event: ChatInitializedEvent):
    ...
    async def _on_new_message(self, event: NewMessageEvent):
        logger.debug(f'[event] NEW_MESSAGE  user={getattr(event.message.user, "username", "?")}  chat={event.chat.id}')
    ...
    async def _on_new_review(self, event: NewReviewEvent):
        logger.debug(f'[event] NEW_REVIEW  deal={event.deal.id}  rating={getattr(event.deal.review, "rating", "?")}')
    ...
    async def _on_review_removed(self, event: ReviewRemovedEvent):
        logger.debug(f'[event] REVIEW_REMOVED  deal={event.deal.id}')
        if event.deal.user.id == self.account.id:
            return
    ...
    async def _on_review_updated(self, event: ReviewUpdatedEvent):
        logger.debug(f'[event] REVIEW_UPDATED  deal={event.deal.id}')
        if event.deal.user.id == self.account.id:
            return
    ...
    async def _on_new_problem(self, event: DealHasProblemEvent):
        logger.debug(f'[event] DEAL_HAS_PROBLEM  deal={event.deal.id}  user={event.deal.user.username}')
        if event.deal.user.id == self.account.id:
            return
        deal = event.deal
        try:
        ...
        if self.config['alerts']['enabled'] and self.config['alerts']['on']['problem']:
            ...
            asyncio.run_coroutine_threadsafe(_get_panel().log_event(text=_log_text(title=f'🤬 Новая жалоба в <a href="https://playerok.com/deal/{deal.id}">сделке</a>', text=body), kb=_log_mess_kb(deal.user.username, _prob_chat)), _get_panel_loop())

    async def _on_deal_problem_resolved(self, event: DealProblemResolvedEvent):
        logger.debug(f'[event] DEAL_PROBLEM_RESOLVED  deal={event.deal.id}  user={event.deal.user.username}')
        if event.deal.user.id == self.account.id:
            return
        did = event.deal.id
        now = time.time()
    ...
    async def _on_new_deal(self, event: NewDealEvent):
        logger.debug(f'[event] NEW_DEAL  user={event.deal.user.username}  item={getattr(event.deal.item, "name", "?")}  deal={event.deal.id}')
        if event.deal.user.id == self.account.id:
            return
        try:
        ...
        if self.config['alerts']['enabled'] and self.config['alerts']['on']['deal']:
            asyncio.run_coroutine_threadsafe(_get_panel().log_event(text=_log_text(title=f'📋 Новая <a href="https://playerok.com/deal/{event.deal.id}">сделка</a>', text=f"<b>Покупатель:</b> {event.deal.user.username}\n<b>Предмет:</b> {event.deal.item.name or '-'}\n<b>Сумма:</b> {event.deal.item.price or '?'}₽"), kb=_log_deal_kb(event.deal.user.username, event.deal.id)), _get_panel_loop())
    ...
    async def _on_item_paid(self, event: ItemPaidEvent):
        logger.debug(f'[event] ITEM_PAID  deal={event.deal.id}  item={getattr(event.deal.item, "name", "?")}')
        if event.deal.user.id == self.account.id:
            return
        if self.config['auto']['restore']['sold']:
    ...
    async def _on_deal_status_changed(self, event: DealStatusChangedEvent):
        logger.debug(f'[event] DEAL_STATUS_CHANGED  deal={event.deal.id}  status={getattr(event.deal.status, "name", "?")}')
        if event.deal.user.id == self.account.id:
            return
        ...
        if self.config['alerts']['enabled'] and self.config['alerts']['on']['deal_changed'] and not confirmed_with_open_problem:
            asyncio.run_coroutine_threadsafe(_get_panel().log_event(_log_text(title=f'🔄️📋 Статус <a href="https://playerok.com/deal/{event.deal.id}/">сделки</a> изменился', text=f'<b>Новый статус:</b> {html.escape(status_frmtd)}')), _get_panel_loop())
    ...
    async def run_bot(self):
        ...
        if proxy:
            ...
            draw_box('АККАУНТ', acc_rows, lead='\n', trail='\n')
        on_sys('ON_READY', Runtime._on_bot_init, 0)
        on_mkt(MarketEvent.CHAT_INITIALIZED, Runtime._on_chat_initialized, 0)
        on_mkt(MarketEvent.NEW_MESSAGE, Runtime._on_new_message, 0)
        on_mkt(MarketEvent.NEW_REVIEW, Runtime._on_new_review, 0)
        on_mkt(MarketEvent.REVIEW_REMOVED, Runtime._on_review_removed, 0)
        on_mkt(MarketEvent.REVIEW_UPDATED, Runtime._on_review_updated, 0)
        on_mkt(MarketEvent.DEAL_HAS_PROBLEM, Runtime._on_new_problem, 0)
        on_mkt(MarketEvent.DEAL_PROBLEM_RESOLVED, Runtime._on_deal_problem_resolved, 0)
        on_mkt(MarketEvent.NEW_DEAL, Runtime._on_new_deal, 0)
        on_mkt(MarketEvent.ITEM_PAID, Runtime._on_item_paid, 0)
        on_mkt(MarketEvent.DEAL_STATUS_CHANGED, Runtime._on_deal_status_changed, 0)

        async def listener_loop():
            feed = Feed(self.account)
            for event in feed.listen():
                await dispatch_market(event.type, [self, event])

        spawn_async(listener_loop)
        await dispatch('ON_READY', [self])
