...
from keel.tone import C_DIM, C_PRIMARY, C_HIGHLIGHT, C_TEXT, C_BRIGHT, C_SUCCESS, C_WARNING, C_ERROR
...
_MODULE_TAGS = {
    ...
    'chamber.sup':   ('BOT ',  Fore.LIGHTCYAN_EX),
    'pl.feed':  ('FEED', Fore.LIGHTBLUE_EX),
    'pl.bus':   ('BUS ',  Fore.LIGHTBLACK_EX),
    'keel.graft':   ('EXT ',  Fore.LIGHTMAGENTA_EX),
    'trellis.face':  ('CTRL', Fore.LIGHTWHITE_EX),
    'pl.util':  ('UTIL', Fore.LIGHTBLACK_EX),
    'pl.cfg':   ('CFG ',  Fore.LIGHTBLACK_EX),
    'app.boot':  ('MAIN', Fore.LIGHTGREEN_EX),
...
_PL_LOGGERS = [
    'pl.conn', 'chamber.sup', 'trellis.face', 'pl.bus', 'keel.graft',
    'pl.util', 'pl.feed', 'pl.cfg', 'app.boot',
]
...
def account_reachable() -> bool:
    try:
        ...
        cfg = ConfigShelf.get('config')['account']
        timeout = cfg.get('timeout') or 30
        SessionGate(token=cfg['token'], user_agent=cfg.get('user_agent') or '', requests_timeout=int(timeout), proxy=cfg.get('proxy') or None).get()
        return True
    except Exception:
        return False
...
def account_banned() -> bool:
    try:
        ...
        cfg = ConfigShelf.get('config')['account']
        timeout = cfg.get('timeout') or 30
        acc = SessionGate(token=cfg['token'], user_agent=cfg.get('user_agent') or '', requests_timeout=int(timeout), proxy=cfg.get('proxy') or None).get()
