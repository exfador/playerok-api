...
class Conn:
    ...
    def _timeout(self) -> int:
        try:
            ...
            t = AppConf.get('config').get('account', {}).get('timeout')
    ...
    def _verbose(self) -> bool:
        try:
            from lib.cfg import AppConf
            return bool(AppConf.get('config').get('debug', {}).get('verbose', False))
        except Exception:
            return False
    ...
    def activate_listing(self, item_id: str, priority_status_id: str, transaction_provider_id: PayGateway = PayGateway.LOCAL) -> types.Item:
        headers = {'accept': '*/*'}
    ...
    def load_boost_tiers(self, item_id: str, item_price: str) -> list[types.ItemPriorityStatus]:
        headers = {'accept': '*/*'}
