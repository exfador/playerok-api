...
class ChatInitializedEvent(BaseEvent):
    ...
    def __init__(self, chat: types.Chat):
        super(ChatInitializedEvent, self).__init__(MarketEvent.CHAT_INITIALIZED, chat)
...
class NewMessageEvent(BaseEvent):
    ...
    def __init__(self, message: types.ChatMessage, chat: types.Chat):
        super(NewMessageEvent, self).__init__(MarketEvent.NEW_MESSAGE, chat)
...
class NewDealEvent(BaseEvent):
    ...
    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super(NewDealEvent, self).__init__(MarketEvent.NEW_DEAL, chat)
...
class NewReviewEvent(BaseEvent):
    ...
    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super(NewReviewEvent, self).__init__(MarketEvent.NEW_REVIEW, chat)
        self.deal: types.ItemDeal = deal


class ReviewRemovedEvent(BaseEvent):

    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super(ReviewRemovedEvent, self).__init__(MarketEvent.REVIEW_REMOVED, chat)
        self.deal: types.ItemDeal = deal


class ReviewUpdatedEvent(BaseEvent):

    def __init__(self, deal: types.ItemDeal, chat: types.Chat, previous_fp: str):
        super(ReviewUpdatedEvent, self).__init__(MarketEvent.REVIEW_UPDATED, chat)
        self.deal: types.ItemDeal = deal
...
class DealConfirmedEvent(BaseEvent):
    ...
    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super(DealConfirmedEvent, self).__init__(MarketEvent.DEAL_CONFIRMED, chat)
        self.deal: types.ItemDeal = deal
...
class DealRolledBackEvent(BaseEvent):
    ...
    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super(DealRolledBackEvent, self).__init__(MarketEvent.DEAL_ROLLED_BACK, chat)
        self.deal: types.ItemDeal = deal
...
class DealHasProblemEvent(BaseEvent):
    ...
    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super(DealHasProblemEvent, self).__init__(MarketEvent.DEAL_HAS_PROBLEM, chat)
        self.deal: types.ItemDeal = deal
...
class DealProblemResolvedEvent(BaseEvent):
    ...
    def __init__(self, deal: types.ItemDeal, chat: types.Chat, resolver_username: str | None = None):
        super(DealProblemResolvedEvent, self).__init__(MarketEvent.DEAL_PROBLEM_RESOLVED, chat)
        self.deal: types.ItemDeal = deal
...
class DealStatusChangedEvent(BaseEvent):
    ...
    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super(DealStatusChangedEvent, self).__init__(MarketEvent.DEAL_STATUS_CHANGED, chat)
        self.deal: types.ItemDeal = deal
...
class ItemPaidEvent(BaseEvent):
    ...
    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super(ItemPaidEvent, self).__init__(MarketEvent.ITEM_PAID, chat)
        self.deal: types.ItemDeal = deal
...
class ItemSentEvent(BaseEvent):
    ...
    def __init__(self, deal: types.ItemDeal, chat: types.Chat):
        super(ItemSentEvent, self).__init__(MarketEvent.ITEM_SENT, chat)
        self.deal: types.ItemDeal = deal
...
class Feed:
        ...
        if message.text == '{{ITEM_PAID}}':
            ...
            if actual_msg and actual_msg.deal:
                ...
                if deal_id not in self.processed_deals:
                    ...
                    return []
                return [NewDealEvent(actual_msg.deal, chat_obj), ItemPaidEvent(actual_msg.deal, chat_obj)]
        elif message.text == '{{ITEM_SENT}}':
            actual_msg = self._get_actual_message(message.id, chat_obj.id) or message
            if actual_msg and actual_msg.deal:
                return [ItemSentEvent(actual_msg.deal, chat_obj), DealStatusChangedEvent(actual_msg.deal, chat_obj)]
            ...
            if actual_msg and actual_msg.deal:
                ...
                if deal_id not in self.review_check_deals and deal_id not in self.review_watch_deals:
                    self.review_check_deals.append(deal_id)
                return [DealConfirmedEvent(actual_msg.deal, chat_obj), DealStatusChangedEvent(actual_msg.deal, chat_obj)]
        elif message.text == '{{DEAL_ROLLED_BACK}}':
            actual_msg = self._get_actual_message(message.id, chat_obj.id) or message
            if actual_msg and actual_msg.deal:
                return [DealRolledBackEvent(actual_msg.deal, chat_obj), DealStatusChangedEvent(actual_msg.deal, chat_obj)]
        elif message.text == '{{DEAL_HAS_PROBLEM}}':
            actual_msg = self._get_actual_message(message.id, chat_obj.id) or message
            if actual_msg and actual_msg.deal:
                return [DealHasProblemEvent(actual_msg.deal, chat_obj), DealStatusChangedEvent(actual_msg.deal, chat_obj)]
        elif message.text == '{{DEAL_PROBLEM_RESOLVED}}':
            actual_msg = self._get_actual_message(message.id, chat_obj.id) or message
            if actual_msg and actual_msg.deal:
                ru = getattr(getattr(actual_msg, 'user', None), 'username', None)
                return [DealProblemResolvedEvent(actual_msg.deal, chat_obj, resolver_username=ru)]
        return [NewMessageEvent(message, chat_obj)]
    ...
    def _proccess_new_chat_message(self, chat_obj, message):
        ...
        if not is_subscribed:
            ...
            if is_new_chat:
                events.append(ChatInitializedEvent(chat_obj))
            ...
            for deal_id in list(self.review_watch_deals):
                try:
                    ...
                    if old and not fp:
                        ...
                        self._resolve_deal_chat(deal)
                        yield ReviewRemovedEvent(deal, deal.chat)
                    elif fp and old and fp != old:
                        self.review_snapshots[deal_id] = fp
                        self._resolve_deal_chat(deal)
                        yield ReviewUpdatedEvent(deal, deal.chat, previous_fp=old)
