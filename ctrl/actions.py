...
async def callback_log_chat_history(callback: CallbackQuery, callback_data: calls.LogChatHistory, state: FSMContext):
    ...
    from bot.core import active_engine, first_link_preview_url
    eng = active_engine()
    if not eng:
...
async def callback_refund_deal(callback: CallbackQuery, state: FSMContext):
    await state.set_state(None)
    from bot.core import active_engine as get_runtime
    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
    eng = get_runtime()
    data = await state.get_data()
...
@router.callback_query(F.data == 'refresh_extension')
...
async def callback_reset_stats(callback: CallbackQuery, state: FSMContext):
    await state.set_state(None)
    from bot.core import set_stats, get_stats, Stats
    s = get_stats()
