...
from aiogram.types import InlineKeyboardMarkup, LinkPreviewOptions, MenuButtonCommands
...
logger = logging.getLogger('pl.ctrl')
...
def get_panel() -> Panel | None:
    if hasattr(Panel, 'instance'):
        return getattr(Panel, 'instance')


def get_panel_loop() -> asyncio.AbstractEventLoop | None:
    if hasattr(get_panel(), 'loop'):
        return getattr(get_panel(), 'loop')


class Panel:

    def __new__(cls, *args, **kwargs) -> Panel:
        if not hasattr(cls, 'instance'):
            cls.instance = super(Panel, cls).__new__(cls)
    ...
    async def run_bot(self):
        ...
        await dispatch('ON_PANEL_READY', [self])
