...
apply_tls_patch()
from lib.consts import VERSION, C_PRIMARY, C_DIM, C_BRIGHT, C_SUCCESS, C_WARNING, C_ERROR, C_TEXT
...
from lib.ext import discover_extensions, register_extensions, activate_extensions
from lib.bus import dispatch

log = logging.getLogger('pl.main')
    ...
    while True:
        ...
        while not config['account']['token']:
            val = _field(
                ...
                [
                    ...
                    '',
                    'Как получить:',
                    '  1. Войдите на playerok.com',
                    '  2. Установите расширение Cookie-Editor',
                    '  3. Нажмите Export → скопируйте поле «token»',
                ],
        ...
        if _blank(config['account']['user_agent']):
            val = _field(
                ...
                [
                    'Идентифицирует ваш браузер при запросах к Playerok.',
                    'Без него авторизация может не пройти.',
                    '',
                    'Узнать свой: https://whatmyuseragent.com',
                    '  скопируйте строку из поля «Your User Agent»',
                    '',
