...
from keel.aliases import cc_get_items, cc_find_by_id, cc_item_summary, KNOWN_EVENTS
...
def settings_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text='✅ Авто-подтверждение', callback_data=calls.SettingsNavigation(to='complete').pack()),
        ],
        [
...
def settings_bump_kb() -> InlineKeyboardMarkup:
    config = cfg.get('config')
    enabled = config['auto']['bump']['enabled']
    rows = [[InlineKeyboardButton(text=_tog('Авто-поднятие', enabled), callback_data='switch_auto_bump_items_enabled')]]
    if not enabled:
...
def settings_complete_kb() -> InlineKeyboardMarkup:
    config = cfg.get('config')
    enabled = config['auto']['confirm']['enabled']
    rows = [[InlineKeyboardButton(text=_tog('Авто-подтверждение', enabled), callback_data='switch_auto_complete_deals_enabled')]]
    if not enabled:
        rows.append([InlineKeyboardButton(text='⬅️ Назад', callback_data=calls.SettingsNavigation(to='index').pack())])
        return InlineKeyboardMarkup(inline_keyboard=rows)
    all_mode = config['auto']['confirm']['all']
...
def settings_restore_text() -> str:
    ...
    return (
        ...
        f'• Платное восстановление (PREMIUM): {_sw(premium)}\n'
...
def settings_restore_kb() -> InlineKeyboardMarkup:
    ...
    rows: list[list] = [
        ...
        [InlineKeyboardButton(text=_tog('💎 Платное (PREMIUM)', premium), callback_data='switch_auto_restore_premium')],
...
def settings_logger_text(chat_id: int | None = None) -> str:
    ...
    return (
        ...
        f'{_row(_alert_effective(enabled, ev, "restore", True), "Авто-восстановление лота")}\n'
        f'{_row(_alert_effective(enabled, ev, "bump", True), "Авто-поднятие лота")}\n'
        ...
        f'{_row(_alert_effective(enabled, ev, "startup", True), "Запуск бота")}'
    )
...
def settings_logger_kb() -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        [
            InlineKeyboardButton(text=_log_btn(_alert_effective(enabled, ev, 'restore', True), 'Авто-восстановление лота'), callback_data='switch_tg_logging_event_restore_ok'),
            InlineKeyboardButton(text=_log_btn(_alert_effective(enabled, ev, 'bump', True), 'Авто-поднятие лота'), callback_data='switch_tg_logging_event_bump_ok'),
        ],
        ...
        [InlineKeyboardButton(text=_log_btn(_alert_effective(enabled, ev, 'startup', True), 'Запуск бота'), callback_data='switch_tg_logging_event_bot_startup')],
        [_mm_btn()],
    ]
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def settings_delivs_kb(page: int = 0) -> InlineKeyboardMarkup:
    ...
    rows = [[InlineKeyboardButton(text=_tog('📦  Авто-выдача', delivs), callback_data='switch_auto_deliveries_enabled')]]
    if not delivs:
        rows.append([InlineKeyboardButton(text='⬅️ Назад', callback_data=calls.SettingsNavigation(to='index').pack())])
        return InlineKeyboardMarkup(inline_keyboard=rows)
